...
//188数码管  
volatile bool T_flag = 0; // 188数码管刷新标志
...
volatile bool LED_flag = 0; // LED按键状态标志
...
//LED按键中断
	...
	TRISA= 0B00101000;		//设置RA55的IO口为输入
	IOCA = 0B00101000;		//允许RA5的IO口电平变化中断
...
void Set_PWM(unsigned char channel, unsigned char duty)
{
	unsigned short pulse_width = (duty * 255) / 100;  // 占空比转换为脉宽值（256为最大值）
	switch (channel) 
	{
		case 0:
    ...
    if (button_sflag)   //短按唤醒
	{
		...
		switch (pwm_duty)
		{
			...
			case 4:
				...
				TRISA = 0B00111111;	//LED，电机对应的口线输入
				break;
			default:
				break;
		}
...
{	
	...
	{
		TMR2IF = 0;
		if(LED_delay>1)//20ms倒计时
		{
			LED_delay--;
		}
		else if(LED_delay==1)//20ms时检测
		{
			LED_delay=0;//关闭20ms定时器
			if (!(PORTB & 0x20))  // RB5 低电平（按下）
			{
				LED_flag = 1;  // 标记LED按键按下
			}
		}
